...
olvidar la edición es la clase de bug que ya se cerró a mano tres veces en este repo. La empresa
de cada evento sale de la ENTIDAD, nunca del header — ver `_audit_payloads_clientes`.
"""
...
class ClienteService:
    ...
    def get_clientes(self, empresa_id: Optional[UUID] = None,
                     incluir_inactivos: bool = False) -> ClienteListResponse:
        """Catálogo de la empresa activa (None = consolidado), por nombre."""
        items = self._repo.find_all(empresa_id, incluir_inactivos)
    ...
    def get_cliente(self, id: UUID, empresa_id: Optional[UUID] = None) -> ClienteResponse:
        """Detalle. 404 IDÉNTICO para "no existe" y "es de otra empresa" — un code o un mensaje
        distinto confirmaría la existencia de clientes ajenos."""
        cliente = self._repo.find_by_id(str(id), empresa_id)
        if not cliente:
            raise AppError(*_NO_ENCONTRADO)
        return cliente

    def create_cliente(self, data: ClienteCreate, usuario_id: Optional[str]) -> ClienteResponse:
        """Alta. La empresa sale del body (ACCIÓN), no del header.
        ...
        if self._repo.existe_nombre(data.nombre, data.empresa_id):
            raise AppError("Ya existe un cliente con ese nombre en la empresa",
    ...
    def update_cliente(self, id: UUID, data: ClienteUpdate,
        ...
        """Edición parcial. Lee el PRIOR antes de escribir: sin él no hay diff que auditar, y
        además es donde se aplica la barrera de empresa antes de tocar nada."""
        ...
        if data.nombre is not None:
            if not data.nombre.strip():
                raise AppError("El nombre es requerido", "NOMBRE_REQUERIDO", 422)
            if self._repo.existe_nombre(data.nombre, prior.empresa_id, excepto_id=str(id)):
                raise AppError("Ya existe un cliente con ese nombre en la empresa",
                               "CLIENTE_DUPLICADO", 409)
    ...
    def delete_cliente(self, id: UUID, empresa_id: Optional[UUID] = None,
                       usuario_id: Optional[str] = None) -> None:
        """Baja LÓGICA. Ver el encabezado del módulo: no hay borrado físico."""
        prior = self._repo.find_by_id(str(id), empresa_id)
        if not prior:
            raise AppError(*_NO_ENCONTRADO)
        if not self._repo.update(str(id), {"activo": False}, empresa_id):
            raise AppError(*_NO_ENCONTRADO)
