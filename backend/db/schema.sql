...
ALTER TABLE public.vacantes ADD CONSTRAINT vacantes_codigo_formato CHECK ((codigo ~ '^[A-Z0-9]+(-[A-Z0-9]+)*$'::text) AND (codigo ~ '[A-Z]'::text) AND (char_length(codigo) >= 3) AND (char_length(codigo) <= 60));  -- migracion 122 + 123: lo escribe Capital Humano en texto natural y la app lo canoniza
