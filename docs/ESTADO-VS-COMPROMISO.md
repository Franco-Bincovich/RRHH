...
- Los dos documentos del directorio (junio 2026): plan de implementación y documento ejecutivo — ninguno de los dos vive en el repo. El inventario de ítems se reconstruyó desde `Plan de trabajo` (raíz del repo), que es el instrumento interno derivado de ellos y enumera Entrega 2, Entrega 3 y el frente AWS.
...
| 1b | Campo **domicilio** | E2 | — | **PARCIAL** | DB `empleados.domicilio` (text, un solo campo) · API `schemas/empleado.py:48,107,170` · UI `modal/_constants.ts:119`, `ficha/DatosEmpleadoSection.tsx:34` | Es **un solo campo de texto libre**, no un domicilio desglosado (calle / número / piso / localidad / CP / provincia). Si el documento comprometía "domicilio completo" estructurado, falta el desglose. |
...
| 3 | **Historial de cambios en el legajo** (rol, área, seniority, sueldo) visible en la ficha | E2 | — | **PARCIAL** | Sí hay vista dentro de la ficha: `frontend/components/features/empleados/ficha/HistorialCambiosSection.tsx` (montada en `app/(dashboard)/empleados/[id]/page.tsx:102`). Se alimenta del audit log filtrado por `entidad="empleado"` + `registro_id={empleadoId}` (`HistorialCambiosSection.tsx:37`), con paginado y modal de detalle. Rol / área / seniority son columnas de `empleados`, así que **sí** caen ahí. | **El sueldo NO aparece.** Vive en `costos_nomina` y se audita bajo `entidad="nomina"`, `registro_id=nomina.id` (`backend/services/_audit_payloads_rrhh.py:64`), que el filtro de la sección no alcanza. El import masivo de nómina audita con `registro_id=uuid4()` de evento (`:182`), tampoco vinculable a un empleado. Falta: traer el histórico de sueldo a la ficha. Además, es el audit log genérico — no hay tabla dedicada de historial ni vista de "antes → después" por campo de negocio. |
...
### 3.4 · "Domicilio completo" (ítem 1b) — DISTINTO si se esperaba desglose
- **El documento dice:** domicilio completo.
- **El sistema hace:** un único campo de texto libre `domicilio`. Sin calle / número / piso / localidad / CP / provincia por separado, así que no es filtrable ni agregable por ubicación.
...
4. **`Plan de trabajo` (raíz) está desactualizado en su ítem 1.4:** afirma que el bloqueo por período "compara contra la fecha de hoy, no contra la fecha del registro". El código actual compara **por solapamiento con el rango del registro** (`services/_periodo_utils.py:36-38,63-72`), o sea que ese defecto ya se corrigió. Lo que sigue vigente de ese ítem es lo otro: solo aplica a `mandos_medios` y el call de costos pasa `rol=None`.
