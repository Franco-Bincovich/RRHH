...
6. **El seniority se guarda ya presentable** (`Senior`, `Semi Senior`) y el campo sigue siendo de
   ...
   primera vez; de ahí en adelante el combobox lo sugiere.
7. Qué se espera que "traten de romper".
...
├── services/            ← 256 archivos de lógica de negocio (284 con submódulos: export/ 10,
...
├── migrations/          ← 121 archivos SQL (000_run_all + 001–123 menos 075–077, que
...
└── tests/               ← 242 archivos .py: 219 `test_*.py` + 23 helpers `_*.py` (exentos del
...
desarrollo pero **`next build` falla**. `vitest` cubre 1717 tests en 152 archivos, pero **la mayor
...
### Tests
- **Backend: 4553 passed** en **219 archivos `test_*.py`** (+ **23 helpers** `tests/_*.py`, que no son tests — 242 archivos `.py` en total dentro de `tests/`). `pytest -q` desde `backend/` con `venv`. *(Remedido el 26/8/2026, en la tanda del código de vacante en texto natural: ningún archivo de test nuevo — los 6 que entran caen en `test_vacante_codigo*.py` y `test_cv_matcher.py`, que ya cubrían esas unidades. La tanda anterior del mismo día lo dejó en 4519 en 219, al pasar el código de vacante al formulario. El archivo nuevo es UNO, `test_vacante_codigo_unico.py`, y existe aparte de `test_vacante_codigo.py` porque cubren unidades distintas: aquél mira la COLUMNA (que el valor viaje al INSERT, que el índice único lo defienda, que llegue al schema) y éste la REGLA de aplicación (normalizar, rechazar nombrando la búsqueda dueña, traducir el choque de la base) — un archivo de test cubre UN módulo. La tanda anterior dejó 4468 en 218, en "los arreglos que hacen daño". Los TRES archivos nuevos son uno por unidad: `test_objetivos_auditoria.py` (los cuatro eventos del CRUD de objetivos), `test_auditoria_destructivas.py` (barrido nº 42) y `test_semilla_alcanza_lo_que_se_escribe.py` (barrido nº 43) — un archivo de test cubre UN módulo. Los DOS helpers nuevos son sus motores: `_barrido_destructivas.py` y `_barrido_tablas.py`. 🟢 Estos números los vigila `tests/test_claude_md_no_miente.py`: ya no se corrigen a mano.)*
...
- **Son 54 barridos estructurales conocidos** (30 backend + 24 front), renumerados el 19/8/2026 —
  ...
  51. 🔴 **`tests/test_legajo_ficha_export.py`** (25/8/2026) — **el export de empleados es un
      SUPERCONJUNTO de la ficha.** Hasta ese día los dos lados divergían en **11 campos** y nada
      lo vigilaba: la ficha mostraba tipo de documento, sexo, fecha de nacimiento, teléfono
      alternativo, email alternativo, domicilio, estudios, ubicación, turno, organismo y perfil, y
      **ninguno de los once salía en el archivo**. Es el peor modo de falla de un export: se abre,
      tiene filas, y le faltan columnas. 🔑 **El eje es el NOMBRE DEL CAMPO, no la etiqueta**, y
      por eso no es otro espejo manual: comparar "Tipo de documento" (la ficha) con el header del
      Excel obligaría a mantener una tabla de traducción a mano. Los dos lados leen el MISMO
      objeto —`empleado.<campo>` en el TSX, `e.<campo>` en el dict del export— así que el atributo
      es un hecho compartido. 🔴 **La unidad "ficha" son CUATRO archivos y elegir mal la unidad
      daba siete falsos positivos medidos**: `nombre`, `apellido` y `estado` viven en la BARRA DE
      IDENTIDAD (§3 los saca del panel a propósito) y las cuatro piezas del domicilio las consume
      `domicilioLegible`. Declararlos como excepciones habría escondido que la ficha sí los
      muestra. Cierra con los cuatro campos que Capital Humano sacó del legajo (organismo,
  ...
  52. 🔴 **`tests/test_marca_un_solo_lugar.py`** (25/8/2026) — **el nombre de la plataforma se
      ...
      de un test del backend que lee el front: el nº 51). Enmascara comentarios y docstrings: este
      repo documenta EN PROSA, y varios archivos explican justamente por qué el nombre no está
      escrito ahí — marcarlos empujaría a borrar la explicación. Un quinto test exige que los DOS
      defaults digan lo MISMO: son dos constantes porque son dos procesos, pero la pantalla y el
      PDF que esa pantalla descarga no pueden decir nombres distintos. ⚠️ **Las migraciones no se
      barren y no es un agujero**: son historia (la 027 sembró "Template Estándar Karstec"),
      reescribirlas no cambia una fila de producción. Verificado por mutación.
  53. 🔴 **`tests/test_columnas_empleados.py`** (25/8/2026) — **toda columna de `empleados` está
      expuesta, declarada con su razón, o es un derivado declarado.** Es el nº 13/14 aplicado por
      fin **a la tabla central del producto**, y llegó tarde: el patrón nació en `candidatos` el
      19/8 y cubría 3 tablas, `empleados` no era una de ellas. El precio se pagó el 25/8, cuando
      medir las 58 columnas contra `EmpleadoResponse` devolvió **DOCE que la base tenía y el
      schema descartaba en silencio, siete de ellas CON DATO en producción** —
      `fecha_ingreso_reconocida` (10/41), `potencial`/`desempeno` (41/41), `liderazgo` y
      `product_owner` (31/41), `equipo`, `co_sourcing`—. La más cara **decide el cupo de
      vacaciones** y no se mostraba en ninguna pantalla. **Sexta aparición del mismo bug**
      (`candidatos.estado` · las 6 de la mig 116 · `fecha_egreso` · `motivo_baja` ·
      `kpis_extra.errores`), y la primera vez que se cubre la tabla de la que cuelga todo.
      🔴 **La premisa se ancla distinto que en los otros tres**: acá el SELECT no es un
      `select("*")` pelado sino la constante `_empleado_row.SELECT`, que arranca con `*` y le
      suma tres embeds — buscar el literal daría verde sobre un archivo que no lo contiene.
      ⚠️ **Lo que NO puede ver, dicho de frente**: una columna que el schema SÍ expone y que
      nadie escribe nunca. `organismo`/`sector`/`perfil` estuvieron 0/41 con la ficha mostrando
      un guion, y este barrido las habría dado por buenas —el schema y la tabla coinciden
      perfecto—. Ése es el bug contrario y pide cruzar contra los ESCRITORES; está anotado.
      Cinco declaradas, todas con disparador: `user_id`, `foto_url`, `updated_at` y las dos
      fechas de trámite de la mig 113, que **no tienen un solo escritor ni lector en el código**.
      Verificado por mutación en las dos direcciones.
  54. 🔴 **`tests/test_filtros_publicados_sin_ui.py`** (25/8/2026) — **todo filtro que un endpoint
      de lectura ACEPTA, o lo manda el front, o está declarado con su razón.** 🔑 **Cierra un
      agujero que los otros dos guardias de filtros no pueden ver POR CONSTRUCCIÓN**: el nº 1
      (`test_paridad_list_export`) y su par del front (`filtros-export.test.ts`) comparan los dos
      **LISTADO contra EXPORT**; ninguno mira backend contra front. Un filtro que el backend acepta
      y que NINGUNA de las dos puntas manda pasa los dos en verde — el par coincide perfecto, en
      cero. Por ahí se colaron **cinco filtros**, encontrados a mano en tres tandas distintas
      (`area` y `periodicidad` de objetivos, éste **con endpoint de catálogo propio sin un solo
      llamador**; y los tres de vacaciones pendientes, cuya tabla ignoraba la barra de filtros de
      su propia pantalla). 🔴 **Normaliza los parámetros de path antes de comparar** (`{lote_id}`
      → `{}`): sin eso toda ruta dinámica se ve como "el front no la llama" y cuatro filtros de
      evaluaciones aparecían huérfanos estando cableados. ⚠️ **El detector SOBRECUENTA a
      propósito** —matchea cualquier `clave:`, incluida la de un tipo— porque la versión estricta
      daba **31 falsos positivos**: el repo arma sus queries de cuatro formas y con traductores
      compartidos, y un barrido con treinta excepciones es uno que nadie mira. La dirección del
      error es la correcta: no acusar de más. Dos listas que NO se mezclan: `DECLARADOS` ("no se
      manda, y está bien" — los 3 params del callback de Google, que los pone Google) y
      `TRADUCE_AFUERA` ("SÍ se manda, el detector no puede verlo"), **cuyas entradas nombran el
      archivo donde vive la clave y hay un test que lo abre y lo comprueba**: es una afirmación
      chequeada, no una excusa. Verificado por mutación contra los cinco casos reales.
