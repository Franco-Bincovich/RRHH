...
export function ItemsTab({ canWrite }: { canWrite: boolean }) {
  ...
  const { items, loading, error, total, load } = useListadoItemsInv(filtros, page, pageSize)
  ...
  async function handleDelete(id: string) {
    setDeletingId(id)
    // 🔴 EL MENSAJE DEL BACKEND SE MUESTRA TAL CUAL. El `catch` genérico que había acá tapaba
    // el 409 `ITEM_ASIGNADO` ("Primero registrá su devolución") con un "Intentá de nuevo", que
    // manda a repetir exactamente lo que no va a funcionar. Mismo criterio que
    // `useActivarEmpleado`: el genérico es sólo para lo que no es un ApiError (la red caída).
    try { await deleteItem(id); await load() }
    catch (e) { toast.error(e instanceof ApiError ? e.message : "No se pudo eliminar el ítem.") }
